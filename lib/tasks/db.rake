namespace :db do
  ...
  file dump_file do
    verify_exec "heroku pg:backups capture --app #{ENV['HA_HEROKU_PRODUCTION_APP_NAME']}"
    verify_exec "curl -o #{dump_file} `heroku pg:backups public-url -a #{ENV['HA_HEROKU_PRODUCTION_APP_NAME']}`"
  end
